...
public class Test5_Payments extends SpringTest {
...
//    private final String jDWrongPassword = new TestRequest(johnDoe).setProps("password", "none").toJson();
     ...
     */
    CheckResult testPostSignUpResponse(String body, String[] role) {
            ...
            if (deniedMethods.contains(entry.getKey())) {
                ...
                if (response.getStatusCode() != HttpStatus.METHOD_NOT_ALLOWED.value()) {
                    return CheckResult.wrong("Method " + entry.getKey().toUpperCase() +
                                             " is not allowed for " + api +
                                             " status code should be 405, responded: " +
    ...
    CheckResult testGetPaymentResponse(String user, String correctResponse) {
    ...
    CheckResult testDeleteAdminApi(HttpStatus status, String user, String param, String answer, String message) {
    ...
    CheckResult testPutAdminApi(HttpStatus status, String user, String reqUser,
    ...
    CheckResult testRoleModelNegative(String api, String method, String user, String message) {
    ...
    DynamicTesting[] dt = new DynamicTesting[]{
            ...
            () -> testPostSignUpResponse(jDCorrectUser, new String[]{"ROLE_ADMINISTRATOR"}),
            () -> testPostSignUpResponse(maxMusLower, new String[]{"ROLE_USER"}),
            () -> testPostSignUpResponse(ivanIvanovCorrectUser, new String[]{"ROLE_USER"}),
            () -> testPostSignUpResponse(petrPetrovCorrectUser, new String[]{"ROLE_USER"}),

            // =============================================================== testing user registration negative tests
            () -> testApi(null, jDCorrectUser, 400, signUpApi, "POST", "User must be unique!"),
            () -> testUserDuplicates(jDCorrectUser),
            () -> testApi(null, jDLower, 400, signUpApi, "POST", "User must be unique (ignorecase)!"),
            ...
            () -> testApi(null, "", 401, getEmployeePaymentApi, "GET", "This api only for authenticated user"),
            ...
            () -> testApi(jDCorrectUser, "", 401, adminApi, "GET", "Password must be changed!"),
            () -> testApi(jDNewPass, "", 200, adminApi, "GET", "Password must be changed!"),
            ...
            // ========================================================================================================
            // ============================================================================================ delete user
            () -> testGetAdminApi(jDNewPass, firstResponseAdminApi,
                    "Api must be available to admin user"),
            () -> testDeleteAdminApi(HttpStatus.OK, jDNewPass, "petrpetrov@acme.com",
                    "Deleted successfully!", "Trying to delete user"),
            () -> testGetAdminApi(jDNewPass, secondResponseAdminApi,
                    "User must be deleted!"),
            () -> testDeleteAdminApi(HttpStatus.BAD_REQUEST, jDNewPass,
                    "johndoe@acme.com", "Can't remove ADMINISTRATOR role!", "Trying to delete admin"),
            () -> testDeleteAdminApi(HttpStatus.NOT_FOUND, jDNewPass,
                    "johndoe@goole.com", "User not found!", "Trying to delete non existing user"),

            // ========================================================================================= changing roles
            () -> testPutAdminApi(HttpStatus.OK, jDNewPass, ivanIvanovCorrectUser,
                    "ACCOUNTANT", "GRANT", new String[]{"ROLE_ACCOUNTANT", "ROLE_USER"}, ""),
            () -> testGetAdminApi(jDNewPass, thirdResponseAdminApi,
                    "Role must be changed!"),
            () -> testPutAdminApi(HttpStatus.OK, jDNewPass, ivanIvanovCorrectUser,
                    "ACCOUNTANT", "REMOVE", new String[]{"ROLE_USER"}, ""),
            () -> testGetAdminApi(jDNewPass, secondResponseAdminApi,
                    "Role must be changed!"),
            () -> testPutAdminApi(HttpStatus.OK, jDNewPass, ivanIvanovCorrectUser,
                    "ACCOUNTANT", "GRANT", new String[]{"ROLE_ACCOUNTANT", "ROLE_USER"}, ""),
            ...
            () -> testPutAdminApi(HttpStatus.NOT_FOUND, jDNewPass, ivanIvanovCorrectUser,
                    "NEW_ROLE", "GRANT", new String[]{"Role not found!"}, "Trying add not existing role!"),
            ...
            () -> testPutAdminApi(HttpStatus.BAD_REQUEST, jDNewPass, jDNewPass,
                    ...
                    "Trying remove administrator role!"),
            ...
            () -> testPostPaymentResponse(ivanIvanovCorrectUser, paymentsList, 200,
                    "Payment list must be added"),
            () -> testGetPaymentResponse(maxMusCorrectUser, correctPaymentResponse
            ),
            () -> testGetPaymentResponse(ivanIvanovCorrectUser, correctPaymentResponseIvanov
            ),
            ...
            () -> testGetPaymentResponseParam(maxMusCorrectUser, 200, updatePayment,
                    updatePaymentResponse, "Salary must be update!"),
            () -> testGetPaymentResponseParam(maxMusCorrectUser, 400, updatePaymentWrongDate,
                    updatePaymentResponse, "Wrong date in request!"),
    };
}